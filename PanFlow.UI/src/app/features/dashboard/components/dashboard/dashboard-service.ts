...
export class DashboardService {
  ...
  // 🎯 1. تحويل التاريخ إلى Signal ليصبح تفاعلياً
  activeDate = signal<Date>(new Date());

  // 🎯 2. استخدام computed ليتحدث اسم اليوم والتاريخ تلقائياً مع اللغة والتاريخ
  dayName = computed(() => {
  ...
  // ── دوال التحكم في الـ Profile والـ Trash ──
  closeProfile() {
  ...
  refreshCard() {
    ...
    // 🎯 قراءة القيمة من الـ Signal باستخدام `()`
  ...
  unlockDay() {
    ...
    this.dayAPI.create({ date: targetDate }).subscribe({
      next: () => {
  ...
  toggleHabit(habit: ReadDayHabitResponse) {
    ...
    this.dayAPI.updateHabitStatus({ dayId: day.dayId, habitId: habit.habitId, isChecked: newStatus }).subscribe({
      next: () => {
        this.currentDay.update((d) => {
          if (!d) return d;
